...
export default {
	...
	argTypes: {
		...
		mode: {
			...
			control: { type: 'select' },
			description:
				'```single``` renders a simple FAB. ```rollout``` renders a FAB that by clicking it, other options roll out.',
			defaultValue: null,
		},
		type: {
			options: [
		...
		onClick: {
			description:
				'The onClick event fired when a FAB, or a FAB item is clicked. When ```mode="single"``` the event is of ```type () => void;``` . When ```mode="rollout"``` the event is of type ```(key: string | number) => void;``` ',
		},
		items: {
			if: { arg: 'mode', eq: 'rollout' },
			description:
				'The properties of the items that will roll out when a ```mode="rollout"``` FAB is clicked',
		},
	},
