...
const Accordion = ({
    ...
    const addContentToRefMap = (ref: HTMLElement | null, key: string | number) => {
        ...
        if (!exists && ref) {
            ...
            ref.style.maxHeight = isExpanded ? `${refContentBbox.height}px` : '0px'
    ...
    const addIconToRefMap = (ref: HTMLElement | null, key: string | number) => {
        ...
        if (!exists && ref) {
            ref.style.transform = isExpanded ? 'rotate(180deg)' : 'rotate(0deg)';
            iconMap.push({ key, ref, isExpanded });
        }
    }

    const multipleExpandsClick = (clickedKey: string | number) => {
        const refPair: AccordionRefMap | undefined = contentMap.find(refPair => refPair.key === clickedKey);
        const iconPair: AccordionRefMap | undefined = iconMap.find(refPair => refPair.key === clickedKey);
        if (refPair && iconPair) {
            if (refPair.ref && iconPair.ref) {
                const refContent = refPair.ref.firstChild as HTMLElement;
                const refContentBbox = refContent.getBoundingClientRect()
                refPair.ref.style.maxHeight = refPair.isExpanded ? '0px' : `${refContentBbox.height}px`;
                iconPair.ref.style.transform = iconPair.isExpanded ? 'rotate(0deg)' : 'rotate(180deg)';
    ...
    const singleExpandClick = (clickedKey: string | number) => {
        ...
        iconMap.forEach(refPair => {
            if (refPair.ref) {
                if (refPair.key === clickedKey) {
                    refPair.ref.style.transform = refPair.isExpanded ? 'rotate(0deg)' : 'rotate(180deg)'
                    refPair.isExpanded = !refPair.isExpanded;
                } else {
        ...
        <div style={{width}}>
            {items?.map((item) => (
                <div key={item.key}>
                    ...
                    >
                        <div className='oxyui__accordion__panel__header'>{item.header}</div>
                        <div>
                            <span 
                                ref={ref => addIconToRefMap(ref, item.key)}
                                className='oxyui__accordion__panel__header__icon'
                            >
                                {accordionIcon ? accordionIcon : <MdKeyboardArrowDown fontSize={25}/>}
                            </span>
                        </div>
                    </div>
                    <div 
