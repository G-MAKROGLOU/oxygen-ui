...
const FAB = (props: FABProps) => {
	...
	React.useEffect(() => {
		const clickAway = (e: MouseEvent) => {
			...
			if (
				rolloutRef.current &&
				!rolloutRef.current.contains(target) &&
				rolloutWrapperRef.current &&
				!rolloutWrapperRef.current.contains(target)
			) {
				setRolloutVisible(false);
			}
		};
	...
	if (props.mode === 'rollout') {
		return (
			<div
				ref={rolloutWrapperRef}
				className={`oxyui__rollout__fab__wrapper oxyui__fab__${props.position}`}
			>
				{rolloutVisible
					? props.items.map((item, i) => (
							...
							>
								<button
									...
									type="button"
									onClick={() =>
										oxyRolloutItemOnClick(item.key)
									}
