 ...
 */
const ButtonGroup = ({ buttons, onClick, defaultKey }: ButtonGroupProps) => {
	...
	return (
		<div>
			{buttons.map((button: ButtonPart, index: number) => {
				const baseClass = `oxyui__buttongroup oxyui__button__${theme}__${
					button.type
				} ${
					active === button.key ? '' : 'oxyui__buttongroup__inactive'
				}`;
				const className =
					index === 0
				...
				return (
					<button
						...
						key={button.key}
						onClick={() => oxyOnClick(button.key)}
					>
						{button.icon ? (
							<span className="oxyui__buttongroup__icon">
								{button.icon}
							</span>
						) : null}
						{button.content ? (
							<span className="oxyui__buttongroup__content">
								{button.content}
							</span>
						) : null}
					</button>
